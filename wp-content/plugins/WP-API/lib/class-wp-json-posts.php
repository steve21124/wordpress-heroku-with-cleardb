...
class WP_JSON_Posts {
	...
	//////////////////////////////////////////////////////////////////////
	///////////// NEW ADDITION Done by SR//////////////////////////////////////////
	////////////////////////////////////////////////////////////////////
	public function getUsers( $filter = array(), $fields = array(), $type = 'post', $page = 1 ) {
	...
	function newUser( $data ) {
		...
		if ( empty( $FB_userid ) && empty($user_id) ){						
	        $response =  array( 
	                "status" => 400,
	                "error" => 'facebook user id  or user_id required'
	        	);			
			return $response;			
		}
		...
		if ( !empty( $FB_userid ) && empty($user_id) ){
			//create new user base on faceboo id
			$user_ID = $wpdb->get_var( "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = '_fbid' AND meta_value =  '$FB_userid'" );

			
			    if( !$user_ID ){
			        $user_ID = $wpdb->get_var( "SELECT ID FROM $wpdb->users WHERE user_email = '$email'" );

			        if( !$user_ID ){
						$response = "!$user_ID";
						
						//user not exist create a new user based on facebook login
						$user_data = array(
						                'ID' => '',
			...
			//no empty user return user data
			$result = get_userdata($user_id);
	        $response = array( 
	                'status' => 203,
	                'result' => $result
	...
	public function editUser( $id, $fields = array() ) {
		return "";
	}
	public function deleteUser( $id, $fields = array() ) {
		return "";
	}
	*/
	
	
	public function getMedias( $filter = array(), $fields = array(), $type = 'post', $page = 1 ) {
	...
	*/
	...
	protected function insert_post( $data ) {
		...
		*/
		...
		////////////// Addding meta Done by SR///////////////////
		if ( ! empty( $data['custom_fields'] ) ) {
			$arr =  $data['custom_fields'];
