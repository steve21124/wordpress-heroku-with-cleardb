...
class WP_JSON_Server {
	...
	public function getRoutes() {
		...
		$endpoints = array(
			...
			'/users'             => array(
				array( array( $posts, 'getUsers' ), self::READABLE ),
				array( array( $posts, 'newUser' ),  self::CREATABLE | self::ACCEPT_JSON ),
			),
			...
			foreach ( $handlers as $handler ) {
				...
				if (isset ( $_FILES ['file'] )) {
				    if ($_FILES ["file"] ["error"] == UPLOAD_ERR_OK) {
				        //error_log ("File Was Uploaded AND OK");
						
						$args = array_merge( $args, array( 'data' => $_POST ) );
						
				    } else {
				        //error_log ("File Was Uploaded but BAD");
				    }
				} else {
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		//error_log("------- POST and wanted-----");
		//error_log(print_r($provided, TRUE)); 
		//error_log(print_r($wanted, TRUE)); 
		//error_log("------- end POST and wanted-----");
		foreach ( $wanted as $param ) {
			...
			elseif ( $param->isDefaultValueAvailable() ) {
				...
				//SR_TODO  comment out this missing parameters
